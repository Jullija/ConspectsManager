...
export const updateUserEdition = async (
  params: UpdateUserEditionParams
): Promise<UserEdition | null> => {
...
export const getUserEditionsByEdition = async (editionId: number): Promise<UserEdition[]> => {
  try {
    const response = await axiosClient.get<UserEdition[]>(`/user-editions/?edition=${editionId}`);
    return response.data;
  } catch (error) {
...
export const postUserEdition = async (
  userId: number,
  editionId: number,
  permissionType: PermissionType
) => {
